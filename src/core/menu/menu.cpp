...
void dynamicStyle() {
    auto accentColor = CONFIGCOL("Visuals>Menu>Menu>Accent");
    auto backgroundColor = CONFIGCOL("Visuals>Menu>Menu>Background");
    auto backgroundSecondaryColor = CONFIGCOL("Visuals>Menu>Menu>Background Secondary");
    auto foregroundColor = CONFIGCOL("Visuals>Menu>Menu>Foreground");

    ImVec4 *colors = ImGui::GetStyle().Colors;

    colors[ImGuiCol_Text]      = foregroundColor;
    colors[ImGuiCol_FrameBg]   = accentColor;
    colors[ImGuiCol_MenuBarBg] = backgroundColor;
    colors[ImGuiCol_CheckMark] = foregroundColor;
    colors[ImGuiCol_Tab]       = backgroundSecondaryColor;
...
void Menu::onSwapWindow(SDL_Window* window) {
    if (!initialised) {
        ...
        style();
        ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF(Quicksand_compressed_data, Quicksand_compressed_size, 16.0f);
        initialised = true;
    }
...
void Menu::drawMenu() {
    ...
    ImGui::SameLine();
    ImGui::PopStyleVar();
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(485, 5));
    ImGui::SameLine();
    ImGui::PopStyleVar();
    ...
    ImGui::GetStyle().Colors[ImGuiCol_Button] = (Menu::tabSelected == 0) ? accentColor : backgroundSecondaryColor;
    if (ImGui::Button("Legit", bWidth)) {
        Menu::tabSelected = 0;
    } ImGui::SameLine();

    ImGui::GetStyle().Colors[ImGuiCol_Button] = (Menu::tabSelected == 1) ? accentColor : backgroundSecondaryColor;
    if (ImGui::Button("Rage", bWidth)) {
        Menu::tabSelected = 1;
    } ImGui::SameLine();

    ImGui::GetStyle().Colors[ImGuiCol_Button] = (Menu::tabSelected == 2) ? accentColor : backgroundSecondaryColor;
    if (ImGui::Button("Visuals", bWidth)) {
        Menu::tabSelected = 2;
    } ImGui::SameLine();

    ImGui::GetStyle().Colors[ImGuiCol_Button] = (Menu::tabSelected == 3) ? accentColor : backgroundSecondaryColor;
    if (ImGui::Button("Misc", bWidth)) {
        Menu::tabSelected = 3;
    }

    ImGui::GetStyle().Colors[ImGuiCol_Button] = accentColor;
    ImGui::PopStyleVar();
    ImGui::Separator();
    ImGui::Separator();
