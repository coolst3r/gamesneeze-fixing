...
void Features::Nightmode::onTonemapController(TonemapController* tonemapController) {
    *tonemapController->useExposureMin_ptr() = CONFIGINT("Visuals>World>World>Nightmode") > 1;
    *tonemapController->useExposureMax_ptr() = CONFIGINT("Visuals>World>World>Nightmode") > 1;
    ...
    *tonemapController->useCustomBloomScale_ptr() = true;
    *tonemapController->customBloomScale_ptr() = static_cast<float>(CONFIGINT("Visuals>World>World>Bloom")) / 100.0f;
}
