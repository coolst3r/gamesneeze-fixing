#include "../../includes.hpp"
#include "features.hpp"
#include <algorithm>

void Features::LegitBot::createMove(Command* cmd) {
    if (!Interfaces::engine->IsInGame()) {
        return;
    }

    if (!(Menu::CustomWidgets::isKeyDown(CONFIGINT("Legit>LegitBot>Default>Key")) || CONFIGBOOL("Legit>LegitBot>Default>Always on"))) {
        return;
    }

    if (Globals::localPlayer == nullptr) {
        return;
    }

    if (Globals::localPlayer->health() == 0) {
        return;
    }

    auto weapon = Interfaces::entityList->activeWeapon(Globals::localPlayer);

    if (weapon == nullptr) {
        return;
    }
   
    auto hitboxes = CONFIGINT("Legit>LegitBot>Default>Hitboxes");
    auto smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>Default>Smoothing") / 5.0f);
    auto FOV = CONFIGINT("Legit>LegitBot>Default>FOV") / 10.0f;
    auto recoilCompensation = CONFIGBOOL("Legit>LegitBot>Default>Recoil Compensation");
    auto aimWhileBlind = CONFIGBOOL("Legit>LegitBot>Default>Aim While Blind");
    auto closestDelta = FLT_MAX;
    auto angleToClosestBone = QAngle{};

    if ((std::find(std::begin(pistols), std::end(pistols), weapon->itemIndex()) != std::end(pistols)) && CONFIGBOOL("Legit>LegitBot>Pistol>Override")) {
        hitboxes = CONFIGINT("Legit>LegitBot>Pistol>Hitboxes");
        smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>Pistol>Smoothing") / 5.0f);
        FOV = CONFIGINT("Legit>LegitBot>Pistol>FOV") / 10.0f;
        recoilCompensation = false;
        aimWhileBlind = CONFIGBOOL("Legit>LegitBot>Pistol>Aim While Blind");
    } else if ((std::find(std::begin(heavyPistols), std::end(heavyPistols), weapon->itemIndex()) != std::end(heavyPistols)) && CONFIGBOOL("Legit>LegitBot>Heavy Pistol>Override")) {
        hitboxes = CONFIGINT("Legit>LegitBot>Heavy Pistol>Hitboxes");
        smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>Heavy Pistol>Smoothing") / 5.0f);
        FOV = CONFIGINT("Legit>LegitBot>Heavy Pistol>FOV") / 10.0f;
        recoilCompensation = false;
        aimWhileBlind = CONFIGBOOL("Legit>LegitBot>Heavy Pistol>Aim While Blind");
    } else if ((std::find(std::begin(rifles), std::end(rifles), weapon->itemIndex()) != std::end(rifles)) && CONFIGBOOL("Legit>LegitBot>Rifle>Override")) {
        hitboxes = CONFIGINT("Legit>LegitBot>Rifle>Hitboxes");
        smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>Rifle>Smoothing") / 5.0f);
        FOV = CONFIGINT("Legit>LegitBot>Rifle>FOV") / 10.0f;
        recoilCompensation = CONFIGINT("Legit>LegitBot>Rifle>Recoil Compensation");
        aimWhileBlind = CONFIGBOOL("Legit>LegitBot>Rifle>Aim While Blind");
    } else if ((std::find(std::begin(smgs), std::end(smgs), weapon->itemIndex()) != std::end(smgs)) && CONFIGBOOL("Legit>LegitBot>SMG>Override")) {
        hitboxes = CONFIGINT("Legit>LegitBot>SMG>Hitboxes");
        smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>SMG>Smoothing") / 5.0f);
        FOV = CONFIGINT("Legit>LegitBot>SMG>FOV") / 10.f;
        recoilCompensation = CONFIGINT("Legit>LegitBot>SMG>Recoil Compensation");
        aimWhileBlind = CONFIGBOOL("Legit>LegitBot>SMG>Aim While Blind");
    } else if ((weapon->itemIndex() == WEAPON_SSG08) && CONFIGBOOL("Legit>LegitBot>Scout>Override")) {
        hitboxes = CONFIGINT("Legit>LegitBot>Scout>Hitboxes");
        smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>Scout>Smoothing") / 5.0f);
        FOV = CONFIGINT("Legit>LegitBot>Scout>FOV") / 10.0f;
        recoilCompensation = false;
        aimWhileBlind = CONFIGBOOL("Legit>LegitBot>Scout>Aim While Blind");
    } else if ((weapon->itemIndex() == WEAPON_AWP) && CONFIGBOOL("Legit>LegitBot>AWP>Override")) {
        hitboxes = CONFIGINT("Legit>LegitBot>AWP>Hitboxes");
        smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>AWP>Smoothing") / 5.0f);
        FOV = CONFIGINT("Legit>LegitBot>AWP>FOV") / 10.0f;
        recoilCompensation = false;
        aimWhileBlind = CONFIGBOOL("Legit>LegitBot>AWP>Aim While Blind");
    } else if ((std::find(std::begin(heavyWeapons), std::end(heavyWeapons), weapon->itemIndex()) != std::end(heavyWeapons)) && CONFIGBOOL("Legit>LegitBot>Heavy>Override")) {
        hitboxes = CONFIGINT("Legit>LegitBot>Heavy>Hitboxes");
        smoothing = 1.0f + (CONFIGINT("Legit>LegitBot>Heavy>Smoothing") / 5.0f);
        FOV = CONFIGINT("Legit>LegitBot>Heavy>FOV") / 10.0f;
        recoilCompensation = CONFIGINT("Legit>LegitBot>Heavy>Recoil Compensation");
        aimWhileBlind = CONFIGBOOL("Legit>LegitBot>Heavy>Aim While Blind");
    }
    
    // TODO: There is probably a better way to do this,
    //       but when I tried making a timer with curtime
    //       it didn't work. This will do just fine.
    if (!aimWhileBlind && Globals::localPlayer->flashDuration() > 2.0f) {
        return;
    }

    // enumerate over players and get angle to the closest player hitbox to crosshair (btw this is shit)
    for (int i = 1; i < Interfaces::globals->maxClients; ++i) {
        auto player = Interfaces::entityList->player(i);

        if (player == nullptr) {
            continue;
        }

        if (!player->hittable()) {
            continue;
        }

        if (!player->visible()) {
            continue;
        }

        Matrix3x4 boneMatrix[128];

        if (!player->getAnythingBones(boneMatrix)) {
            continue;
        }

        auto model = Interfaces::modelInfo->GetStudioModel(player->model());

        if (!model) {
            continue;
        }
        
        auto localEyePos = Globals::localPlayer->eyePos();

        // swap this for for loop of bones selected by player
        for (int i = 0; i < 5; ++i) {
            if (hitboxes & 1 << i) {
                // map hitboxes enum to "actual" hitboxes
                auto hitbox = (1 << i & (int)HitBoxes::HEAD   ) ? 8 :
                              (1 << i & (int)HitBoxes::NECK   ) ? 7 :
                              (1 << i & (int)HitBoxes::CHEST  ) ? 6 :
                              (1 << i & (int)HitBoxes::STOMACH) ? 5 :
                              (1 << i & (int)HitBoxes::PELVIS ) ? 3 : -1;

                auto targetBonePos = player->getBonePos(hitbox);
                auto angleToCurrentBone = calcAngle(localEyePos, targetBonePos) - cmd->viewAngle - (recoilCompensation ? Features::NoVisualRecoil::aimPunch * 2.0f : QAngle{});

                normalizeYaw(angleToCurrentBone.y);

                if (angleToCurrentBone.Length() > closestDelta) {
                    continue;
                }

                Features::RageBot::target = player;
                closestDelta = angleToCurrentBone.Length();
                angleToClosestBone = angleToCurrentBone;
            }
        }
    }

    if (closestDelta < FOV) {
        if (((angleToClosestBone) / smoothing).Length() > 0.005f) { // prevent micro-movements
            cmd->viewAngle += (angleToClosestBone) / smoothing;
        }
    }
}
